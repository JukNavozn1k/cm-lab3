...
Данная версия считает с учётом точности
...
def newton_interpolation(x, x_values, y_values, accuracy=1e-6):
  n = len(x_values)
  ...
  for i in range(n):
    ...
    for j in range(i):
      term *= (x - x_values[j])
    result += term
  if abs(result - y_values[-1]) <= accuracy:
    return result
  else:
    return None
...
for x in y_values:
  ...
  if y is not None:
    ...
    print(f'Could not interpolate f({x:.4f}) with the desired accuracy.')
